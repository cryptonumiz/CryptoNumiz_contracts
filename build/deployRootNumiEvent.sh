...
CONTRACT_NAME=RootNumiEvent
giver=0:b5e9240fc2d2f1ff8cbb1d1dee7fb7cae155e5f6320e585fcc685698994a19a5
function giver {
    tonos-cli --url $NETWORK call $giver sendTransaction '{"dest":"'$1'","value":1000000000000,"bounce":false}' --abi ../giver/giver.abi.json --sign ../giver/giver.keys.json  
}
...
function gen_user_code {
    tvm_linker decode --tvc ./NumiEvent.tvc > code.txt
}
...
function deploy_root {
    tonos-cli --url $NETWORK deploy ./$CONTRACT_NAME.tvc '{"root_public_key":"0x'$TVM_ROOT_PUBKEY'","event_code":"'$TVM_USER_CODE'"}' --abi ./$CONTRACT_NAME.abi.json --sign $CONTRACT_NAME.keys.json
}
